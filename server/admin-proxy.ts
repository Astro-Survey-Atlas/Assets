...
export function proxyAdmin(request: IncomingMessage, response: ServerResponse, backend: string): void {
  const target = new URL(backend);
  const transport = target.protocol === "https:" ? https : http;
  const upstream = transport.request({ protocol: target.protocol, hostname: target.hostname, port: target.port,
    path: request.url, method: request.method, headers: { ...request.headers, host: target.host }, timeout: 120_000 }, remote => {
