...
export async function loadPublicState(catalog: LoadedCatalog) {
  ...
  for (const product of products) {
    ...
    layers.set(g.layerId,{layerId:g.layerId,productId:c.productId,surveyId:c.surveyId,releaseId:c.releaseId,product:c.name,modality:c.modality,coverageRole:c.coverageRole,color:c.publicSurvey?.color??"#376b9b",availableOrders:orders,overviewOrder,maxOrder:moc.maxOrder,cellCount:count,areaDeg2:count*41252.96124941927/(12*4**orders.at(-1)!),tileScheme:"ipix-range-4096",cells,
  ...
  for(const surveyId of [...new Set(products.map(p=>p.content.surveyId))]) {
    ...
    surveys.push({id:surveyId,name:meta?.name??surveyId,mission:meta?.mission??surveyId,description:meta?.description??"",color:meta?.color??"#376b9b",modalities:(meta?.modalities??[]) as PublicSurveyModality[],releases,imageUrl:`/surveys/${surveyId}.png`,statistics:{publicProducts:selected.length,acquired:selected.filter(p=>p.geometry).length,overviewOnly:0,awaitingGeometry:selected.filter(p=>!p.geometry).length,notApplicable:0,footprintCells:footprints.filter(f=>f.surveyId===surveyId).reduce((s,f)=>s+f.pixels.length,0)},assets:assets.filter(a=>a.surveyId===surveyId)});
  }
