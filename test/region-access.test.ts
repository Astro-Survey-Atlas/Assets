...
test("region results remain bounded and never infer scientific files from geometry",async t=>{
 ...
 const answer=await queryRegion(state,request,async()=>{throw Error("must not call an absent index");});assert.deepEqual(answer.sources[0]!.cells,[163327]);assert.deepEqual(answer.sources[0]!.downloads,[]);assert.equal(answer.sources[0]!.accessAvailability,"geometry-only");assert.equal(answer.sources[0]!.completeness,"incomplete");assert.match(String(answer.sources[0]!.reason),/No region-to-science-file index/);assert.ok(Date.parse(answer.expiresAt)-Date.now()<=600000);
