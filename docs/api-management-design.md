...
`POST /api/v1/coverage/reverse-lookup` 重合区域完整来源反查及下载计划文件导出，请求头为：
...
“下载全部区块”指依次查询所选重合区块并导出 JSON/CSV 下载计划文件，内容为巡天、
Tile/block/文件身份、匹配依据和来源链接，不包含科学数据，也不会请求链接中的科学文件。
这里的重合区块不等同于来源系统的科学数据 block。顺序查询避免超过并发上限。
Key 不授权管理、审核、发布或签发 Key，也不用于 URL query 参数。
匿名重合预览通过 `preview: true` 请求，仅返回最多 6 条 Tile/文件/来源入口；匿名请求不能携带 cursor 续页。预览响应的 `page.nextCursor` 可作为首个 API Key 鉴权完整页的起点；之后返回的 cursor 绑定该 Key identity。完整反查不接受匿名请求。
抽屉“继续浏览”分页及下载计划文件导出需要 API Key，经 `/api/v1/access/unlock` 验证后
...
授权不会生成原本缺失的科学文件索引，也不授予外部数据源访问权限。Assets 只返回已有
匹配和来源信息；用户自行访问来源获取科学数据，Assets 不代下载。
