...
function renderConnectorDetails(connector?: Connector): void {
  ...
  detail.querySelector<HTMLButtonElement>("[data-delete-connector]")?.addEventListener("click", async (event) => {
    ...
    try {
      await api(`/api/v1/admin/connectors/${encodeURIComponent(connector.name)}`, { method: "DELETE" });
      connectorProbeResults.delete(connector.name);
      renderConnectors(connectorRecords.filter(value => value.name !== connector.name));
      toast("连接已删除，源文件和已有产物已保留");
      await refresh();
    } catch (error) {
      toast(error instanceof Error ? error.message : "删除失败", true);
      renderConnectorDetails(connectorRecords.find(value => value.name === connector.name));
    }
  });
