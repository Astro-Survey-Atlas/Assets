...
function renderFeaturedSurveys(surveys: SurveyRecord[]): void {
  ...
  featured.forEach((survey) => {
    ...
    const precisionNode = document.createElement("small");
    precisionNode.className = "snapshot-precision";
    precisionNode.textContent = orders.length ? `O${Math.max(...orders)}` : "—";
    precisionNode.title = precision;
    precisionNode.setAttribute("aria-label", precision);
    ...
    const releaseCount = survey.releases.length;
    count.textContent = `${releaseCount} DR · ${productCount} ${t("home.productsCount")}`;
    row.append(heading, precisionNode, details, count);
    host.append(row);
  });
  renderIcons(host);
}
