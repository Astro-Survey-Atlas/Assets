...
export const OVERLAP_DOWNLOAD_HEADER = [
  ...
  "source_file_id", "file_name", "file_type", "size_bytes", "source_uri", "downloadable", "download_url", "matching_cells", "coverage_methods", "entrypoint_kind", "tile_id", "entrypoint_url",
...
export function overlapCsvRows(
  ...
  plan.entrypoints.forEach((entry) => {
    ...
    rows.push([
      component.id, "entrypoint", String(entry.order ?? component.order), String(entry.nside ?? 2 ** component.order), entry.precision,
      entry.layerId ?? "", entry.surveyId ?? layer.surveyId, entry.releaseId ?? layer.releaseId, entry.product ?? layer.product, layer.modality,
      "", "", "", "", "", "", "", JSON.stringify(entry.cells ?? component.cells), "", entry.kind,
      typeof entry.tileId === "string" ? entry.tileId : "", entry.url ?? entry.sourceUrl ?? entry.mocUrl ?? "",
